...
function sleepMoodMonthly(){const n=daysIn(state.year,state.month);const weekdays=['Su','Mo','Tu','We','Th','Fr','Sa'];let g='<div class="vertical-grid"><div></div><div class="hdr">Sleep</div><div class="hdr">Mood</div>';for(let d=1;d<=n;d++){const date=`${mk()}-${pad(d)}`,v=db.sleepMood[date]||{};const si=['2–3','4–5','6–7','>8'].indexOf(v.sleep)+1;const wd=weekdays[new Date(state.year,state.month,d).getDay()];g+=`<div class="daynum">${d} <span style="font-size:.72em;font-weight:500;opacity:.62">${wd}</span></div><div class="sq ${si?'sleep-'+si:''}" onclick="openSleepMood('${date}')"></div><div class="sq" onclick="openSleepMood('${date}')">${v.mood?`<img class="mood-thumb" src="${moodImg(v.mood)}">`:''}</div>`}g+='</div>';return `<div class="sm-layout"><div class="card">${g}</div><div class="card"><h3>Sleep (hours)</h3><div class="legend-list">${[['2–3','sleep-1'],['4–5','sleep-2'],['6–7','sleep-3'],['>8','sleep-4']].map(x=>`<div class="legend-row"><i class="legend-swatch ${x[1]}"></i><span>${x[0]}</span></div>`).join('')}</div><hr style="border:0;border-top:1px solid #e6edf1;margin:14px 0"><h3>Mood</h3><div class="legend-list">${moodList.map(m=>`<div class="legend-row"><img class="legend-face" src="${moodImg(m[1])}"><span><strong>${m[0]}</strong><br><span class="subtle">${m[2]}</span></span></div>`).join('')}</div></div></div>`}
