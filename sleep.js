function renderSleepMood(){
 const nav=state.smView==='monthly'?monthNav():yearNav();
 ...
 <div class="tabs tracker-switch"><button class="${state.smView==='monthly'?'active':''}" onclick="state.smView='monthly';renderSleepMood()">Monthly</button><button class="${state.smView==='annual-sleep'?'active':''}" onclick="state.smView='annual-sleep';renderSleepMood()">Annual Sleep</button><button class="${state.smView==='annual-mood'?'active':''}" onclick="state.smView='annual-mood';renderSleepMood()">Annual Mood</button></div>${nav}
 ${state.smView==='monthly'?sleepMoodMonthly():annualView(state.smView==='annual-sleep'?'sleep':'mood')}</section>`;
}
function yearNav(){return `<div class="month-nav"><button onclick="state.year--;renderSleepMood()">‹</button><strong>${state.year}</strong><button onclick="state.year++;renderSleepMood()">›</button></div>`}
