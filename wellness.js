...
function wellnessEnsureData(){
 ...
 let migrated=false;
 db.wellnessHistory=db.wellnessHistory.map(x=>{
  if(x&&typeof x==='object'&&!x.id){migrated=true;return {...x,id:uid()}}
  return x;
 });
 if(migrated)save();
}
...
function wellnessHistoryRow(x){
 ...
 return `<button type="button" class="card wellness-history-row" data-id="${escapeHTML(x.id||'')}" onclick="openWellnessHistoryEntry(this.dataset.id)"><div><strong>${type}</strong><span>${wellnessDateLabel(x.date)}</span></div><div class="wellness-history-copy">${x.reason?`<b>${escapeHTML(x.reason)}</b>`:''}${x.doctor?`<small>${escapeHTML(x.doctor)}</small>`:''}${refs?`<small>Referral: ${escapeHTML(refs)}</small>`:''}${x.prescriptions?`<small>Prescription: ${escapeHTML(x.prescriptions)}</small>`:''}</div><div class="wellness-history-cost">${Number(x.cost)>0?fmtMoney(x.cost):''}</div></button>`;
}

function openWellnessHistoryEntry(id=''){
 wellnessEnsureData();
 const x=id?db.wellnessHistory.find(r=>String(r.id)===String(id)):null;
 const rec=x||{id:'',date:iso(today),type:'doctor',reason:'',doctor:'',referrals:[],otherReferral:'',prescriptions:'',cost:'',notes:''};
 modal(`<h2>${x?'Edit':'Add'} health history</h2><div class="form-grid wellness-history-form-grid"><div class="field"><label>Date</label><input id="whDate" type="date" value="${rec.date||iso(today)}"></div><div class="field"><label>Entry type</label><select id="whType"><option value="doctor" ${rec.type==='doctor'?'selected':''}>Doctor consult</option><option value="blood" ${rec.type==='blood'?'selected':''}>Blood test</option><option value="health" ${rec.type==='health'?'selected':''}>Other health check</option></select></div><div class="field full"><label>Reason for visit / check</label><input id="whReason" value="${escapeHTML(rec.reason||'')}" placeholder="Reason for visit"></div><div class="field full"><label>Doctor</label><input id="whDoctor" value="${escapeHTML(rec.doctor||'')}" placeholder="Doctor or clinic"></div><div class="field full"><label>Diagnostic-test referral</label><div class="check-grid wellness-referrals" id="whReferrals">${[['blood','Blood'],['urine','Urine'],['other','Other']].map(r=>`<button type="button" class="check-choice ${(rec.referrals||[]).includes(r[0])?'active':''}" data-v="${r[0]}" onclick="this.classList.toggle('active')">${r[1]}</button>`).join('')}</div></div><div class="field full"><label>Other referral details</label><input id="whOtherReferral" value="${escapeHTML(rec.otherReferral||'')}" placeholder="e.g. ultrasound, imaging"></div><div class="field full"><label>Prescriptions (and what for)</label><input id="whPrescriptions" value="${escapeHTML(rec.prescriptions||'')}" placeholder="Medication / reason"></div><div class="field"><label>Cost</label><input id="whCost" type="number" min="0" step="0.01" inputmode="decimal" value="${rec.cost??''}"></div><div class="field full"><label>Notes</label><textarea id="whNotes">${escapeHTML(rec.notes||'')}</textarea></div></div><div class="btn-row">${x?`<button class="btn secondary" onclick="deleteWellnessHistory('${escapeHTML(rec.id)}')">Delete</button>`:''}<button class="btn secondary" onclick="closeModal()">Cancel</button><button class="btn" onclick="saveWellnessHistory('${x?escapeHTML(rec.id):''}')">Save</button></div>`)
}
function saveWellnessHistory(id=''){
 ...
 const i=db.wellnessHistory.findIndex(x=>String(x.id)===String(id));if(i>=0)db.wellnessHistory[i]=rec;else db.wellnessHistory.push(rec);save();closeModal();state.route='wellness';state.wellnessView='history';renderWellness();
}
function deleteWellnessHistory(id){if(!confirm('Delete this health-history entry?'))return;db.wellnessHistory=db.wellnessHistory.filter(x=>String(x.id)!==String(id));save();closeModal();state.route='wellness';state.wellnessView='history';renderWellness()}
